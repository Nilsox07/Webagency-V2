 ...
 */
export function PageHeroScene({ accent = "#B6FF3B" }: { accent?: string }) {
  ...
  return (
    <>
      ...
      <pointLight position={[-4, -2, 2]} intensity={20} color="#9BE01F" distance={20} />
      ...
      <group ref={group}>
        ...
        <Torus ref={ring} args={[2.3, 0.015, 16, 90]} rotation={[Math.PI / 3, 0, 0]}>
          <meshStandardMaterial color="#B6FF3B" emissive="#B6FF3B" emissiveIntensity={0.6} />
        </Torus>
      </group>

      <Sparkles count={50} scale={[12, 6, 5]} size={2} speed={0.25} color="#CCFF68" opacity={0.6} />
    </>
  );
}
