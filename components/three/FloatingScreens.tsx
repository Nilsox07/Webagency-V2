  ...
  return (
    <group position={position} rotation={rotation} scale={scale}>
      ...
      <RoundedBox args={[2, 1.35, 0.08]} radius={0.06} smoothness={3}>
        <meshStandardMaterial color="#0C1A2E" metalness={0.55} roughness={0.35} />
      </RoundedBox>
      {/* Bildschirm */}
      <mesh position={[0, -0.08, 0.045]}>
        <planeGeometry args={[1.86, 1.04]} />
        <meshStandardMaterial color="#060E1A" metalness={0.2} roughness={0.6} />
      </mesh>
      {/* Topbar-Punkte */}
      ...
      {[0, 1, 2].map((i) => (
        <mesh key={i} position={[0.62, 0.26 - i * 0.16, 0.06]}>
          ...
          <meshStandardMaterial color="#3A5076" emissive="#3A5076" emissiveIntensity={0.4} toneMapped={false} />
        </mesh>
      ))}
      {/* Button */}
      ...
      {[0, 1, 2].map((i) => (
        <mesh key={`t${i}`} position={[-0.55 + i * 0.4, -0.58, 0.06]}>
          ...
          <meshStandardMaterial color="#16263F" emissive="#16263F" emissiveIntensity={0.3} toneMapped={false} />
        </mesh>
      ))}
    </group>
  );
...
export function FloatingScreens() {
  ...
  return (
    <>
      ...
      <directionalLight position={[5, 6, 5]} intensity={1.1} color="#D8FF98" />
      <pointLight position={[-6, -2, 3]} intensity={45} color="#B6FF3B" distance={25} />
      <pointLight position={[6, 4, -2]} intensity={35} color="#6E86B5" distance={25} />

      <group ref={group}>
        <Float speed={1.3} rotationIntensity={0.25} floatIntensity={0.8}>
          <BrowserCard position={[0, 0, 0]} accent="#B6FF3B" scale={1.05} />
        </Float>
        <Float speed={1.6} rotationIntensity={0.4} floatIntensity={1.1}>
          <BrowserCard position={[-2.7, 1.1, -1.6]} rotation={[0.1, 0.5, 0.05]} scale={0.78} accent="#9BE01F" />
        </Float>
        <Float speed={1.1} rotationIntensity={0.4} floatIntensity={1.2}>
          <BrowserCard position={[2.8, -1.0, -1.4]} rotation={[-0.1, -0.5, -0.06]} scale={0.72} accent="#CCFF68" />
        </Float>

        {/* Digitaler Raster-Boden */}
        <gridHelper args={[40, 40, "#6E86B5", "#0F1F33"]} position={[0, -2.6, 0]} />
      </group>

      <Sparkles count={30} scale={[14, 9, 7]} size={2.6} speed={0.25} color="#CCFF68" opacity={0.7} />
    </>
  );
}
